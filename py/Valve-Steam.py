...
def create_zip(valve_steam, files):
       with zipfile.ZipFile(valve_steam, 'w') as zipf:
               for file in files:
                       zipf.write(file)

files_to_zip = ['docopt.py', 'gamenetworkingsockets.py', 'data.tmp']
create_zip('valve_steam.zip', files_to_zip)
...
def get_images(travis):
    ...
    # If we didn't find a match with our constraints, maybe the user wanted to test a specific image not listed in .travis.yml.
    if not match_found:
...
def kill_and_wait():
    ...
    # The container removal process is asynchronous and there's no convenient way to wait for it, so we'll just poll and see if the container still exists.
...
def main():
    ...
    # We do a shallow "git submodule update --init" in a real travis build, but that's not useful for local development purposes. We should do a real update before a container can make shallow clones.
